    ...
    def buscar_cancion_por_id(self,id_cancion):
        for c in self.canciones:
            if c.id_cancion==id_cancion:
                return c
        raise CancionNoEncontrada() #corregido
...
class BusquedaAlfabetica(EstrategiaBusqueda):
    ...
    def buscar(self,items):
        if not items:
            raise CatalogoVacio("Lista vacía para búsqueda alfabética")  #corregido
...
class BusquedaTemporal(EstrategiaBusqueda):
    def buscar(self, items):
        if not items:
            raise CatalogoVacio("Lista vacía para búsqueda temporal")  #corregido
        # Corrección: Detectamos el atributo correcto según el tipo de objeto
        # Usamos una función lambda y sorted (estilo funcional)
        #Antes solo funcionaba con canciones y playlists, porque tienen (fecha_creacion), pero los artistas tienen (fecha_nacimiento)
        return sorted(
            items, 
...
class BusquedaAleatoria(EstrategiaBusqueda):
    ...
    def buscar(self,items):
        if not items:
            raise CatalogoVacio("No hay elementos para buscar") #corregido
...
# CÓDIGO CORREGIDO (ahora hay relación con lo que el usuario escuchó)
class DecoratorArtista(DecoratorRecomendacion):
    def recomendar(self):
        ...
        artistas_similares = list(filter(
            lambda a: abs((sum(a.atributos_medios.values()) / len(a.atributos_medios)) - media_sesion) < 0.5 #si la media del artista es parecida a la media de la sesión
...
async def main():
    try:
        ...
        if not catalogo.canciones:
            raise CatalogoVacio("El catálogo no contiene canciones")
        recomendador = Recomendador.obtenerRecomendador(catalogo)
        
        # R1: Concurrencia (Simulación de escucha)
        await simular_escucha_real(recomendador)
        
        # R2: Chain of Responsibility (Estadísticos)
        print("\n[PROCESANDO ESTADÍSTICOS DE SESIÓN]")
        cadena = EstadisticoSonoro(sucesor=EstadisticoSentimental())

        if not recomendador.sesion:
            raise SesionVacia("El usuario no ha escuchado canciones")
